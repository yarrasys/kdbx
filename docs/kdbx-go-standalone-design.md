...
|----|-------|------------------------------------|
| `init` ✦ | `--here`, `--mode standard\|strict` (N6) | create vault+keyfile; **bootstrap**: `--here`, or plain `init` with no pointer anywhere above (previously a guaranteed NotFound, so no working invocation changes meaning), first writes a minimal pointer in cwd — project = dir name, `dev`+`prod` envs, `defaultEnv: dev` (C1) — then proceeds; plain `init` bound to a pointer in a *parent* dir keeps its meaning (cloned-repo case) but names the pointer file it is about to use and **confirms on a TTY first** (no TTY and no confirmation → NotConfirmed, exit 4, like `rekey`), with the note pointing at `--here`; `--mode` records the policy in the pointer, and strict anchors it into the new vault; stderr `created <vault>` + KEYFILE backup warning; vault/keyfile/pointer exists → refuse (≠0, Preflight) |
